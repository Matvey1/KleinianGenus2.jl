...
* `Periods` -- some of vectors that belong to the period lattice of the curve (type -- `Matrix{Complex{T}}`). It is a matrix of the size `2 x 2` or `2 x 4` containing a basis of a Lagrange subgroup in the period lattice or a whole basis in the period lattice respectively (see more detailed explanation below).
